...
// -------- OLED screens --------
void drawPlaying(const char* songName) {
  ...
  display.print("I2S: BCLK=7 WS=8 DO=9");
...
bool playWavFile(const char* path) {
  ...
  // show playing screen
  const char* songName = strrchr(path, '/');
  songName = songName ? songName + 1 : path;
  drawPlaying(songName);
